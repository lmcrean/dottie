...
export async function getUserProfile(page: Page, state: TestState): Promise<UserProfileResult> {
  try {
    ...
    const profileContainer = page.locator('[data-testid="user-profile"]').or(
    ...
    const usernameField = page.locator('[data-testid="username"]').or(
      page.locator('input[name="username"]')
    );
    
    const emailField = page.locator('[data-testid="email"]').or(
