...
export async function updateUserProfile(page: Page, state: TestState, updateData: UpdateProfileData): Promise<UpdateUserProfileResult> {
  try {
    ...
    const editButton = page.locator('button:has-text("Edit")').or(
      page.locator('[data-testid="edit-profile"]')
    );
    ...
    if (updateData.displayName) {
      const displayNameField = page.locator('input[name="displayName"]').or(
        page.locator('[data-testid="displayname-input"]')
      );
