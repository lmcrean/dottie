/**
 * Send Message Endpoint Runner
 * 
 * Handles sending messages to AI chat for integration tests.
 */
...
export async function sendMessage(request, token, message, conversationId = null) {
  ...
  if (response.status() !== 200) {
    ...
    throw new Error(`Failed to send message: ${response.status()}. Response: ${responseText}`);
  }
