    ...
    default: {
        ...
        if (table === "assessments") {
          ...
          if (Array.isArray(data)) {
            data.forEach((symptom) => {
              const symptomId = `symptom-${Date.now()}-${Math.random()
                .toString(36)
                .substring(2, 9)}`;
        ...
        if (arguments.length === 2) {
          ...
          if (column === "id") {
            ...
            if (column.assessment_id)
              this._whereAssessmentId = column.assessment_id;
          }
      ...
      then: vi.fn(function (callback) {
        if (this._whereUserId) {
          ...
          const userAssessments = Array.from(mockAssessments.values()).filter(
            (a) => a.user_id === this._whereUserId
          );
          return Promise.resolve(callback(userAssessments));
        } else if (this._whereAssessmentId) {
          // Get symptoms for an assessment
          const assessmentSymptoms = Array.from(mockSymptoms.values()).filter(
            (s) => s.assessment_id === this._whereAssessmentId
          );
          return Promise.resolve(callback(assessmentSymptoms));
        } else if (this._whereAssessmentIds) {
          // Get symptoms for multiple assessments
          const assessmentSymptoms = Array.from(mockSymptoms.values()).filter(
            (s) => this._whereAssessmentIds.includes(s.assessment_id)
          );
          return Promise.resolve(callback(assessmentSymptoms));
        }
...
describe("Assessment Success Integration Tests", () => {
  ...
  test("6. Update Assessment - PUT /api/assessment/:id", async () => {
    try {
      const updateData = {
        assessmentData: {
          ...
          symptoms: {
            ...
            physical: [
              ...testAssessmentData.assessmentData.symptoms.physical,
              "Insomnia",
            ],
          },
        },
      ...
      expect(response.status).toBe(200);
      expect(response.body).toHaveProperty("id", testAssessmentId);
      expect(response.body.assessmentData).toHaveProperty(
        "painLevel",
        "severe"
      );
      expect(response.body.assessmentData.symptoms.physical).toContain(
        "Insomnia"
      );
    } catch (error) {
