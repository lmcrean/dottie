  ...
  try {
    ...
    const decryptedmessage = (messageContent) => {
      if (isLikelyEncrypted(messageContent)) {
        return decryptMessage(decryptedUserKey, messageContent);
      } else {
    ...
    if (
      conversation.assessment_object &&
      typeof conversation.assessment_object === "object"
    ) {
      try {
        ...
        if (conversation.assessment_object.other_symptoms) {
          conversation.assessment_object.other_symptoms =
            ParseAssessmentJson.parseOtherSymptoms(
              conversation.assessment_object.other_symptoms
            );
        }
    ...
    if (mergedOptions.includeMessages) {
      ...
      const endIndex = mergedOptions.limit
        ? startIndex + mergedOptions.limit
        : undefined;
      const selectedMessages =
        mergedOptions.limit || mergedOptions.offset
    ...
    return {
      ...
      pagination: {
        ...
        hasMore: mergedOptions.limit
          ? mergedOptions.offset + mergedOptions.limit < messageCount
          : false,
      },
    };
  } catch (error) {
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        logger.warn(
          "Failed to parse assessment object fields in summary:",
          parseError
        );
        // Continue with unparsed data rather than failing
      }
...
export { getConversation, getConversationForUser, getConversationSummary };
