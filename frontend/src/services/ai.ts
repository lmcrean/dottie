...
const getMockFeedback = (userData: UserData, userMessage?: string): string => {
  ...
  if (userData.age && parseInt(userData.age) > 0) {
    ...
    if (age < 18) {
      response +=
        '• Your cycle is still establishing itself, which is normal for your age. Irregularity is common.\n';
    } else if (age > 40) {
      response += '• As you approach perimenopause, cycle changes may become more noticeable.\n';
    } else {
      response +=
        '• Your cycle should be relatively stable at your age, though some variation is normal.\n';
    }
  }
  ...
  if (userData.cycleLength) {
    ...
    if (cycleLength < 21) {
      response +=
        '• Your cycle is shorter than average. While this can be normal for you, tracking consistency is helpful.\n';
    } else if (cycleLength > 35) {
      response +=
        '• Your cycle is longer than average. If this is a recent change, consider discussing with your healthcare provider.\n';
    } else {
  ...
  if (userData.periodDuration) {
    ...
    if (duration > 7) {
      response +=
        '• Your period duration is longer than average. Consider discussing with your healthcare provider.\n';
    } else {
  ...
  if (userData.flowHeaviness) {
    if (userData.flowHeaviness.toLowerCase().includes('heavy')) {
      response +=
        "• For heavy flow, ensure you're getting enough iron-rich foods and staying hydrated.\n";
    }
  }
  ...
  if (userMessage) {
    ...
    if (userMessage.toLowerCase().includes('pain') || userMessage.toLowerCase().includes('cramp')) {
      ...
      userMessage.toLowerCase().includes('pms') ||
      userMessage.toLowerCase().includes('mood')
    ) {
      response +=
        'Mood changes before your period are common due to hormonal fluctuations. Regular exercise, adequate sleep, and stress management techniques may help. Severe mood changes that affect your quality of life should be discussed with a healthcare provider.';
    } else {
      response +=
        'I recommend tracking your symptoms consistently to identify patterns. This information can be valuable when discussing with healthcare providers.';
    }
  }
