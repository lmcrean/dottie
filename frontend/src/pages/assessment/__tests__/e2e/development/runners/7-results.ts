...
export const checkResultsPage = async (page: Page): Promise<void> => {
  ...
  try {
    // Verify title
    const title = await page.locator('h1').filter({ hasText: /Assessment Results/ }).first();
    await expect(title).toBeVisible();
    console.log("Found Assessment Results title");
    
    // Check for determined pattern section
    const patternSection = await page.locator('div').filter({ hasText: /Your Menstrual Pattern/ }).first();
